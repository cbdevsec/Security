...
function initGridCanvas(){
  ...
  function step(){
    ...
    for (const n of nodes){
      ctx.fillStyle = 'rgba(196,48,43,0.55)';
      ctx.beginPath();
...
function initProjectVisuals(){
  document.querySelectorAll('canvas[data-visual]').forEach(canvas => {
    ...
    function radar(){
      ...
      ctx.closePath();
      ctx.fillStyle = 'rgba(196,48,43,0.22)';
      ctx.fill();
      ctx.restore();

      ctx.strokeStyle = 'rgba(196,48,43,0.9)';
      ctx.beginPath();
      ctx.moveTo(cx, cy);
    ...
    function scan(){
      ...
      gradient.addColorStop(0, 'rgba(196,48,43,0)');
      gradient.addColorStop(0.5, 'rgba(196,48,43,0.4)');
      gradient.addColorStop(1, 'rgba(196,48,43,0)');
    ...
    function grid(){
      ...
      for (let i = 0; i < cols; i++){
        for (let j = 0; j < rows; j++){
          ...
          ctx.fillStyle = `rgba(196,48,43,${0.05 + pulse * 0.18})`;
